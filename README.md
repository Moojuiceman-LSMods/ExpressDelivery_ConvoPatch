...
Patch for the ExpressDelivery mod. Currently if Winston tp's and you sleep, he reverts to talking about the skip delivery. This may get patched in the game itself so rather than risk breaking the original mod with a fix that might stop working, here is a patch that can be added separately and just deleted if the game is updated.
